...
The logging for these disconnections was:

`Peer is stalling block download, disconnecting <peer>`

The ping times of the affected peers were often 20-100 seconds.

I wondered if that algorithm could be optimized, and brought it up on the
bitcoin-core-dev IRC channel here:
https://bitcoin-irc.chaincode.com/bitcoin-core-dev/2025-01-22#1083973

Even after IBD was completed, I still see the following disconnections occurring
with addnode peers:
...
When an addnode peer is disconnected by our IBD headers/blocks download timeout
...
When these disconnection/reconnection cycles happen frequently with our
...
disconnections + immediate reconnections of addnode peers, both during IBD and
afterward.
...
5. What do you think of the review suggestion in
   https://github.com/bitcoin/bitcoin/pull/32051#issuecomment-2825824998 to
   rate peers and use that to scale the number of blocks we request from them?

6. Related to the review feedback in
   https://github.com/bitcoin/bitcoin/pull/32051#pullrequestreview-2691505746,
   how would you implement clearing their block requests? (I'm willing to pull
   in your commit, if it is good!)

7. Do you have any missing test coverage for these cases that you would like to
   propose? (Happy to include it, if you do.)

8. Any other questions or suggestions?
