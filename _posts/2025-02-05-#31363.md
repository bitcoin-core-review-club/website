...
transactions and edges represent UTXO spending relationships. A transaction's _cluster_ is the part of the graph that it
is connected to.
...
### TxGraph Design and Usage

While the current `CTxMemPool` class does not contain a data structure dedicated to storing the graph representation
...
The rebased [cluster mempool implementation PR](https://github.com/bitcoin/bitcoin/pull/28676) shows usage of `TxGraph`
...
A `TxGraph` has a vector of `Entry`s, where each entry is a single transaction, and a vector of `Cluster`s.  It also has
...
4. `ApplyDependencies`: Apply new dependencies.
5. Re-linearize clusters.
...
- When a `TxGraph` is oversized, mutation functions (like `RemoveTransaction` and `AddDependency`) and
  `GetIndividualFeerate` can still be called, but inspection functions like `` are unavailable because we won't
linearize clusters that are oversized.
...
Much of the `TxGraph` members are grouped together as a `ClusterSet`: ther vector of `Cluster`s, work queues, and
counter. This refactor is done so that a `TxGraph` can have multiple **subgraphs** or **levels** represented as
...
This section includes the rest of #31363, but also #31444.
...
1. Under what circumstances could `GetIndividualFeerate` and `GetChunkFeerate` return nothing (i.e. an empty `FeeFrac`?)
   Is it possible that one returns an answer and the other doesn't? How?

1. Why is `AddTransaction` annotated with `[[nodiscard]]`?

1. After calling `TxGraph::RemoveTransaction`, is the transaction removed transaction?
...
1. What is the purpose of supporting staging? In what situation would we use it and does it warrant this complexity?
