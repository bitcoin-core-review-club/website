...
It may be useful to look at the code surrounding mempool to understand why each of the functions:
...
1. Can `TxGraph` more accurately enforce topology restrictions, or is it a just refactor?
...
1. For the `QualityLevel`s `NEEDS_RELINEARIZE`, `ACCEPTABLE`, and `OPTIMAL`, what can / can't you do with a
...
1. What does `PullIn` do, and when is it necessary? How many times can [this loop](https://github.com/bitcoin-core-review-club/bitcoin/commit/79647fd58dc7152eb1dc4e09cc56db845608352d#diff-c25ebcc6e4dc9c9bb21462f796fbb470cee3f0ff1e77bfc4033f4e8460f4482cR790-R797) run?

1. Why is it true that the `m_oversized` value for staging matches that of main? Why is calling `SplitAll()` necessary?

1. How many levels are possible?

1. Which of the following combinations are possible: main oversized, staged oversized), (main oversized, staged not
   oversized), (main not oversized, staged oversized), (main not oversized, staged not oversized).

#### Specialized Functionality

1. Why are these functions necessary? Can you think of a slimmer interface to serve these purposes?
