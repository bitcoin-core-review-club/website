...
- A _conflicted_ transaction is one where the inputs to that transactions have been
  spent by a different transaction which has been confirmed in the block chain.

- Prior to [PR #3669](https://github.com/bitcoin/bitcoin/pull/3669), the Bitcoin Core
  wallet did not track conflicted transactions. If a transaction sending to or from
  the wallet was conflicted, it would continue to be displayed in the wallet as
  unconfirmed forever.
...
- [PR 3694](https://github.com/bitcoin/bitcoin/pull/9694) notified the wallet directly
  when [a transaction left the mempool because it became conflicted with a
  transaction in a newly connected
...
- Accurately tracking when a transaction is conflicted is difficult. Consider the case
  ...
  the `AddToWalletIfInvolvingMe()` function.
