...
- Monitoring transactions as they leave the mempool. If a transaction is confirmed in a block, the estimator records success in the bucket, along with the number of blocks it took to prove (i.e., the difference between the confirmation block height and the height at which it was first seen in the mempool).
...
- **Conservative** – Uses long-term data for more cautious estimates and is tailored toward users who cannot afford to RBF bump later, hence high fee rate estimates are often seen.
