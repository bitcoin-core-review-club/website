...
- PR [#22363](https://github.com/bitcoin/bitcoin/pull/22363) replaces many manually-constructed
  default scripts in the functional tests with helper functions provided in `script_util.py` and
`wallet_util.py`. It also corrects an error in the helper function,
...
2. What do `key_to_p2pkh_script`, `script_to_p2sh_script`, `key_to_p2wpkh_script` and
    `script_to_p2wsh_script` in wallet\_util.py do? In what cases would we want to use or not use them?
...
11. Can you find any other places in functional tests that could use the wallet\_util.py helper
